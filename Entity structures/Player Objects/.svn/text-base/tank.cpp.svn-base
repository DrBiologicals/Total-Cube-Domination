...
#include "../particle/picparticleemitter.h"
...
, m_bTurret(true)
, m_fMineCoolDown(0.0f)
, m_bIsMineCool(true)
, m_bAimed(false)
, m_bMoved(false)
, m_bShot(false)
, m_fRespawnCameraTimer(0.0f)
, m_fRespawnCameraEnd(7.0f)
...
, m_fDamaged(0)
{
	...
	memset(&m_vec2LastTargetPosition, 0, sizeof(m_vec2LastTargetPosition));
	memset(&m_vec2NextTargetPosition, 0, sizeof(m_vec2NextTargetPosition));
}
				  ...
				  uint32 _uiShieldTextureID)
{
	...
	m_uiShieldTextureID = _uiShieldTextureID;
...
CTank::Process(float32 _fDeltaTick)
{
	...
	if(GetAlive() == true)
	{
		if (!m_bShutDown)
		{
			...
			if(true == m_bIsMovingToTarget && false == m_bTransitioning)
			{
				MoveToTarget(_fDeltaTick);
			}
			...
			{
				if (m_fShieldRechargeTimer > 2.5f)
				{
		...
		if(true == HasFlag())
		{
			...
			m_pFlag->SetVectors(m_vec3Up, m_vec3Right, m_vec3Direction, m_vec3Position);
		...
		if (dynamic_cast<CGameState*>(CTotalCubeDomination::GetInstance().GetProgramState())->IsNewRound() && !m_bTransitioning)
		{
			Die();
			Reset();
		}
	...
	//This is for respawning the player
	//This is just to make it look more interesting
	if(!GetAlive())
	{
		m_fRespawnCameraTimer += _fDeltaTick;
		if(m_fRespawnCameraTimer >= m_fRespawnCameraEnd)
		{
			m_fRespawnCameraTimer = 0.0f;
...
CTank::Draw()
{
	...
	bool bShowInvis = false;
	if(m_ePowerUpType == POWERUP_INVISIBILITY)
	{
		bShowInvis = true;
		if(m_fPowerUpTimer>=8.0f)
		{
			int32 iVal = static_cast<int32>(m_fPowerUpTimer*6);
			if(iVal % 2)
			{
				bShowInvis = false;
			}
	...
	else if(m_fDamaged > 0)
	{
		D3DXMATRIX matShieldWorld;
		D3DXMatrixIdentity(&matShieldWorld);
		matShieldWorld._11 += 2.2f;
		matShieldWorld._22 += 2.2f;
		matShieldWorld._33 += 2.2f;
		matShieldWorld._41 += m_matWorld._41;
		matShieldWorld._42 += m_matWorld._42;
		matShieldWorld._43 += m_matWorld._43;
		pRenderer->SetWorldMatrix(matShieldWorld);
		...
		CTextureManager::GetInstance().SetTexture(0, m_uiShieldTextureID);
		CModelManager::GetInstance().Draw(m_uiOverShieldModelID);
...
CTank::Die()
{
	...
	m_vec3MoveEffectDirection = D3DXVECTOR3(0.0f, 0.0f, 0.0f);
	m_bOnFaultyMagPlate = false;
...
CTank::Reset()
{
    if (m_pEMP)
    {
        m_pEMP->SetAlive(false);
    }

    if (m_pMine)
    {
        m_pMine->SetActive(false);
    }
...
* @author Rebeccah Cox - changed from moving the tank, to setting a target position instead.
...
{
	if(GetAlive() && !GetDisabled() && false == m_bIsMovingToTarget)
	{
        if (!m_bTransitioning &&
			m_CurrentAngle.GetAngle() == m_TargetAngle.GetAngle())
		{
			...
			CRules* pRules = dynamic_cast<CGameState*>(CTotalCubeDomination::GetInstance().GetProgramState())->GetRules();
			if (m_bMoved == false)
			{
				m_bMoved = true;
			}
			if (m_sDirection == DIRECTION_UP)
			{
				...
				if (m_TargetAngle.GetAngle() == m_CurrentAngle.GetAngle())
				{
					...
					{
						// Set next target to move to.
						m_vec2LastTargetPosition = m_vec2NextTargetPosition;

						m_vec2NextTargetPosition.y = m_vec2Position.y - 1.0f;
			...
			else if (m_sDirection == DIRECTION_LEFT)
			{
				...
				if (m_TargetAngle.GetAngle() == m_CurrentAngle.GetAngle())
				{
					float32 fX1 = m_vec2Position.y - fTankHalfWidth;
					float32 fX2 = m_vec2Position.y + fTankHalfWidth;

					D3DXVECTOR2 vec2Corner1(m_vec2Position.x, fX1);
					D3DXVECTOR2 vec2Corner2(m_vec2Position.x, fX2);
						...
						&&(CPathfinder::GetInstance().IsConnecting(m_sDirection, vec2Corner2, m_eFacePosition)))
					{
						...
						m_vec2NextTargetPosition.x = m_vec2Position.x - 1.0f;
...
/**
* 
* Moves the tank towards it's target position.
* If the tank has reached the target position, it is centered on the tile and sets 
*	m_bIsMovingToTarget to false to signal that it can now receive a new target position.
* 
* @author Rebeccah Cox
* @param float32 _fDeltaTick
*/
void
CTank::MoveToTarget(float32 _fDeltaTick)
{
	CPathfinder::ConvertUnit3DtoTile2D(m_eFacePosition, m_vec3Position, m_vec2Position);

	float32 fPlateModifier = D3DXVec3Dot(&m_vec3Direction, &m_vec3MoveEffectDirection);

	float32 fSpeed = (_fDeltaTick * (((HasFlag())?(0.8f):(1.0f)) * (m_fMovementSpeed + 0.05f * m_iMoveSpeedIncreasePIC * m_fMovementSpeed) + fPlateModifier * 4.5f)) * 0.3333333f;

	switch (m_sDirection)
	{
		case DIRECTION_UP:
		{
			if((m_vec2Position.y - fSpeed) > m_vec2NextTargetPosition.y)
			{
				m_vec2Position.y -= fSpeed;
			}
			else
			{
				m_vec2Position.y = m_vec2NextTargetPosition.y;
				m_bIsMovingToTarget = false;
			}
			break;
		}
		...
		case DIRECTION_LEFT:
		{
			if((m_vec2Position.x - fSpeed) > m_vec2NextTargetPosition.x)
			{
				m_vec2Position.x -= fSpeed;
			}
			else
			{
				m_vec2Position.x = m_vec2NextTargetPosition.x;
				m_bIsMovingToTarget = false;
			}
			break;
		}
...
/**
* 
* This moves the tank to the center of the next tile after changing a face.
* 
* @author Rebeccah Cox
*/
void
CTank::UpdateTargetPosition()
{
	switch (m_eFacePosition)
	{
		case FACE_TOP:
		{
			if(FACE_FRONT == m_eTargetFace)
			{
				m_vec2NextTargetPosition.y = 0.5f;
			}
			break;
		}
		case FACE_BOTTOM:
		{
			if(FACE_BACK == m_eTargetFace)
			{
				m_vec2NextTargetPosition.y = 14.5f;
			}
			break;
		}
		case FACE_FRONT:
		{
			if(FACE_TOP == m_eTargetFace)
			{
				m_vec2NextTargetPosition.y = 14.5f;
			}
			if(FACE_LEFT == m_eTargetFace)
			{
				m_vec2NextTargetPosition.x = 14.5f;
			}
			if(FACE_RIGHT == m_eTargetFace)
			{
				m_vec2NextTargetPosition.x = 0.5f;
			}
			break;
		}
		case FACE_BACK:
		{
			if(FACE_BOTTOM == m_eTargetFace)
			{
				m_vec2NextTargetPosition.y = 0.5f;
			}
			if(FACE_LEFT == m_eTargetFace)
...
CTank::SetPIC(EPIC _ePIC, int32 _iPICValue)
{
	...
	if(_iPICValue > 0)
	{
		uiID = CEmitterManager::GetInstance().AddEmitter(1, m_vec3Position, PARTICLE_PIC, m_eTeam, 0, 0, this);
		dynamic_cast<CPICParticleEmitter*>(CEmitterManager::GetInstance().GetEmitter(uiID))->SetPicType(_ePIC);
	}
}
...
CTank::Respawn(const TPoint& _tLocation)
{
	...
	CPathfinder::ConvertUnit3DtoTile2D(m_eFacePosition, m_vec3Position, m_vec2Position);
	m_vec2LastTargetPosition = m_vec2Position;
	m_vec2NextTargetPosition = m_vec2Position;

	m_sDirection = DIRECTION_NONE;
...
/*
* Checks the cooldown of the mine and sets stuff 
* This is all put into here to save space in the process
*
* @author Michael McQuarrie
* @param _fDeltaTick
* 
...
*/
void
CTank::CheckMineCool(float32 _fDeltaTick)
{
	if(GetMine() == 0)
	{
		m_fMineCoolDown += _fDeltaTick;
		if(m_fMineCoolDown >= 5.0f)
		{
			SetMineCool(true);
			m_fMineCoolDown = 0.0f;
		}
...
void
CTank::SetMineCool(bool _bCool)
{
	m_bIsMineCool = _bCool;
}

/*
* Gets the Mine Cool bool
*
* @author Michael McQuarrie
* 
* @return m_bIsMineCool
*/
bool
CTank::GetMineCool()
{
...
/*
* This returns the has aimed bool
* @author Rigardt de Vries
* @return m_bAimed
*/
bool
...
/*
* This returns the has moved bool
* @author Rigardt de Vries
* @return m_bMoved
*/
bool
...
/*
* This returns the has shot bool
* @author Rigardt de Vries
* @return m_bShot
*/
bool
...
* sets the aimed bool
* @author Rigardt de Vries
* @param _bAimed			the new aimed bool value
* @return void
*/
void
CTank::SetAimed(bool _bAimed)
{
	m_bAimed = _bAimed;
}